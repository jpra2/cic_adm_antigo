...
M1= MeshManager(inputfile + '.msh')          # Objeto que armazenará as informações da malha
...
for meshset in meshsets_nv2:
    ...
    for child in childs:
        M1.mb.add_parent_meshet(child, meshset)
    elems = M1.mb.get_entities_by_handle(meshset)
...
gravity_tag = M1.mb.tag_get_handle('GRAVITY', 1, types.MB_TYPE_BIT, types.MB_TAG_SPARSE)
M1.mb.tag_set_data(gravity_tag, 0, M1.gravity)
mi_tag = M1.mb.tag_get_handle('MI', 1, types.MB_TYPE_DOUBLE, types.MB_TAG_SPARSE)
M1.mb.tag_set_data(mi_tag, 0, 1.0)
gamma_tag = M1.mb.tag_get_handle('GAMMA', 1, types.MB_TYPE_DOUBLE, types.MB_TAG_SPARSE)
M1.mb.tag_set_data(gamma_tag, 0, 10.0)



M1.mb.write_file(inputfile + '_out.h5m')
m = M1.mb.create_meshset()
M1.mb.add_entities(m, M1.all_volumes)
M1.mb.write_file(inputfile + '_out.h5m', [m])
